...
const colorFields: {
  key: keyof WebsiteSettings["colors"];
  label: string;
}[] = [
...
const typographyFields: {
  key: Exclude<keyof WebsiteSettings["typography"], "lineHeight">;
  label: string;
  min: number;
  max: number;
}[] = [
...
export function SettingsManager() {
  ...
  function updateTypography(
    key: keyof WebsiteSettings["typography"],
    value: string
  ) {
    setSettings((current) => ({
      ...current,
  ...
  return (
    <>
      <AdminPageHeader
        ...
        description="Manage the public website theme, default mode, logo and typography from one place."
        action={
          <button
      ...
      {loading ? (
        <AdminLoading label="Loading website settings" />
      ) : (
        <form
        ...
        >
                ...
                <div className="admin-mode-options">
                  {(["dark", "light"] as const).map((mode) => (
                    <label
                      key={mode}
                      className={
                        settings.defaultMode === mode ? "active" : undefined
                      }
                    >
                      <input
                        type="radio"
          ...
          <section className="admin-panel admin-settings-panel">
            ...
            <div className="admin-settings-grid typography-grid">
              {typographyFields.map(({ key, label, min, max }) => (
                <label className="admin-range-field" key={key}>
                  <span>{label}</span>
                  <div>
                    <input
                      type="range"
                      min={min}
                      max={max}
                      value={pxToNumber(settings.typography[key])}
                      onChange={(event) =>
                        updateTypography(key, `${event.target.value}px`)
                      }
                    />
                    <input
                      type="number"
              ...
              <label className="admin-range-field">
                ...
                <div>
                  <input
                    ...
                    onChange={(event) =>
                      updateTypography("lineHeight", event.target.value)
                    }
                  />
                  <input
                    type="number"

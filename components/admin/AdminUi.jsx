...
export function AdminModal({ open, title, description, children, onClose, width = "720px", }) {
    return (<AnimatePresence>
      {open && (<motion.div className="admin-modal-layer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <button type="button" className="admin-modal-backdrop" onClick={onClose} aria-label="Close dialog"/>
          <motion.section role="dialog" aria-modal="true" aria-label={title} className="admin-modal" style={{ maxWidth: width }} initial={{ opacity: 0, y: 24, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.98 }} transition={{ duration: 0.24 }}>
            <header>
              <div>
                <h2>{title}</h2>

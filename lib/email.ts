...
type BookingEmailData = Omit<BookingInput, "time"> & {
  time: string;
...
function bookingRows(booking: BookingEmailData) {
  const rows = [
    ["Client", booking.name],
    ["Email", booking.email],
    ["Phone", booking.phone],
...
function emailFrame(title: string, intro: string, booking: BookingEmailData) {
...
function calendarAttachment(booking: BookingEmailData) {
...
export async function sendBookingCreatedEmails(booking: BookingInput) {
  ...
  await Promise.all([
    ...
    transporter.sendMail({
      ...
      html: emailFrame(
        "Your request is in",
        "Thanks for reaching out. We received your booking and will confirm the next step shortly.",
        emailBooking
      ),
      attachments: [attachment],
    }),
...
export async function sendBookingUpdatedEmails(booking: AdminBookingInput) {
  const { config, transporter } = createTransporter();
  const statusLabel =
  ...
  await Promise.all([
    transporter.sendMail({
      ...
      html: emailFrame(
        "Booking updated",
        "The admin dashboard saved the following booking changes.",
        booking
      ),
      attachments: [attachment],
    }),
    transporter.sendMail({
      ...
      html: emailFrame(
        ...
        `Your booking status is now ${statusLabel}. The latest details are below.`,
        booking
      ),
      attachments: [attachment],
    }),
...
export async function sendMeetingScheduledEmails(booking: AdminBookingInput) {
  const { config, transporter } = createTransporter();
  const attachment = calendarAttachment(booking);
  ...
  await Promise.all([
    transporter.sendMail({
      ...
      html: emailFrame(
        "Meeting scheduled",
        "A meeting was created from the hike agency admin dashboard.",
        booking
      ),
      attachments: [attachment],
    }),
    transporter.sendMail({
      ...
      html: emailFrame(
        ...
        "Your meeting details are confirmed below. Please keep this email for your records.",
        booking
      ),
      attachments: [attachment],
    }),

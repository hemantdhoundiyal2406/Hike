...
import { mergeThemeSettings, websiteSettingsSchema, } from "@/lib/theme-schema";
...
export async function saveWebsiteSettings(value) {
    const settings = websiteSettingsSchema.parse(value);
    await connectToDatabase();
    const updated = await WebsiteSettingsModel.findOneAndUpdate({ key: "global" }, {
        $set: {
            key: "global",
    ...
    return mergeThemeSettings(updated);
}

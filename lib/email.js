...
function getMailConfig() {
    ...
    const host = readEnv(ENV_ALIASES.host);
    const portValue = readEnv(ENV_ALIASES.port);
    const user = readEnv(ENV_ALIASES.user);
    const pass = readEnv(ENV_ALIASES.pass);
    ...
    return {
        ...
        host,
        port,
        secure: parseBoolean(readEnv(ENV_ALIASES.secure), port === 465),
        user,
        pass,
        from,
        fromName,
...
function createTransporter() {
    ...
    const transportOptions = config.service && !config.host
        ? {

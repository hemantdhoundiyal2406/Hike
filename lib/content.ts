...
import type { ProjectRecord, TestimonialRecord } from "@/types/content";
...
function fallbackTestimonials(): TestimonialRecord[] {
...
export async function getPublicProjects() {
  if (!process.env.MONGODB_URI) return fallbackProjects();

  try {
    await connectToDatabase();
    const projects = await Project.find().sort({ featured: -1, createdAt: -1 }).lean();
    return JSON.parse(JSON.stringify(projects)) as ProjectRecord[];
  } catch (error) {
...
export async function getPublicTestimonials() {
  if (!process.env.MONGODB_URI) return fallbackTestimonials();

  try {
    await connectToDatabase();
    const testimonials = await Testimonial.find().sort({ createdAt: -1 }).lean();
    return JSON.parse(JSON.stringify(testimonials)) as TestimonialRecord[];
  } catch (error) {
